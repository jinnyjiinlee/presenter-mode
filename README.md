...
> [!TIP]
> 데모 슬라이드 6장이 곧 튜토리얼이에요. 방향키로 넘기며 노트 강조 → `B` 블랙아웃 → `4` `Enter` 점프를 순서대로 눌러보세요.
...
| 강사 노트 — `⏰`주황 `⚠️`빨강 `백업`파랑 자동 강조 + 다음 노트 미리보기 | 4초 뒤 커서·힌트 자동 숨김 | 숫자+`Enter` 점프 |
| 필름스트립 — 클릭 이동 · 노트 있는 장 ✎ 배지 | 도움말·점프창 청중 노출 차단 | 두 창 자동 동기화 (2초 내 자동 복구) |
| 타이머 — 목표 설정 → 남은 시간·5분 전 알림, 창 닫아도 유지 | Wake Lock (화면 꺼짐 방지) | 마지막 장에서 안 튐 |
| 현재 시각 · 체류 시간 · 연결 상태 ●/○ · 🌙 다크 토글 | 탭 제목에 진행 상황 (3/96) | `Home`/`End` · `F5` |
...
[내 노트북]   발표자 창 (#presenter) — 노트 · 타이머 · 다음 슬라이드
     ...
     │  (전부 브라우저 안 — 인터넷 불필요)
     ▼
[빔프로젝터]  슬라이드 창 — 슬라이드만 전체화면
```

- 같은 HTML 파일을 **창 2개**로 열어요. URL 해시가 `#presenter`면 발표자 UI, 아니면 슬라이드를 렌더링해요.
- 발표자 창이 2초마다 ping, 슬라이드 창이 pong — 연결 상태 표시(●/○)와 어긋남 자동 복구의 원리예요.
