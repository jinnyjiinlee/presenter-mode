...
| 강사 노트 — `⏰`주황 `⚠️`빨강 `백업`파랑 자동 강조 | 4초 뒤 커서·힌트 자동 숨김 | 숫자+`Enter` 점프 |
| 다음 노트 미리보기 · 노트 접기 | 도움말·점프창 청중 노출 차단 | 두 창 자동 동기화 (2초 내 자동 복구) |
| 타이머 — 목표 설정 → 남은 시간·5분 전 알림 | Wake Lock (화면 꺼짐 방지) | 마지막 장에서 안 튐 |
| 현재 시각 · 체류 시간 · 연결 상태 ●/○ | | |
| 밝은 배경 기본, 🌙 다크 토글 | | |
...
```
┌── 내 노트북 ─────────────────┐         ┌── 빔프로젝터 ────────────┐
│  발표자 창  (#presenter)      │   동기화  │  슬라이드 창              │
│  노트 · 타이머 · 다음 슬라이드 │ ◀─────▶ │  슬라이드만 전체화면       │
└──────────────────────────────┘         └──────────────────────────┘
        localStorage + BroadcastChannel — 전부 브라우저 안 (인터넷 불필요)
```
