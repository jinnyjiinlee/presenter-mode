...
> [!TIP]
> 데모 슬라이드 7장이 곧 튜토리얼이에요. 방향키로 넘기며 노트 강조 → `B` 쉬는 시간 화면 → `D` 판서 → `M` 돋보기 → `4` `Enter` 점프를 순서대로 눌러보세요.
...
| 필름스트립 — 클릭 이동 · 노트 있는 장 ✎ 배지 | ☕ 쉬는 시간 화면 — 로고·안내문·QR·시계·BGM 커스터마이즈 | 두 창 자동 동기화 (2초 내 자동 복구) |
| 타이머 — 목표 설정 → 남은 시간·5분 전 알림, 창 닫아도 유지 | 🔍 돋보기 — 마우스 주변 확대, 휠로 배율 | 마지막 장에서 안 튐 |
| 현재 시각 · 체류 시간 · 연결 상태 ●/○ · 🌙 다크 토글 | 4초 뒤 커서·힌트 숨김 · Wake Lock · 도움말 청중 노출 차단 | `Home`/`End` · `F5` |

### ☕ 쉬는 시간 화면 커스터마이즈 (v5)

`B`를 누르면 그냥 검은 화면 대신, 웨비나 대기·브레이크 타임용 안내 화면을 띄울 수 있어요. `<script>` 안 `BREAK_SCREEN` 설정만 바꾸면 돼요.

```js
const BREAK_SCREEN={
  title:'☕ 잠깐 쉬어가요',          // 큰 제목 — 전부 비우면 순수 검은 화면
  sub:'10분 뒤에 다시 시작할게요',    // 부제목
  logoText:'DAYFOCUS LAB',          // 텍스트 로고 (이미지는 logoImg에 경로)
  logoImg:'',                       // 예: 'logo.png'
